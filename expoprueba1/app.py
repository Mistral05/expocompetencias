...
def perfil():
    ...
    try:
        with conexion.cursor() as cursor:
            ...
            sql_check="SELECT dias, ultimoingreso FROM usuarios WHERE id_usuario = %s"
            cursor.execute(sql_check,(usuario_id,))
            datos_log=cursor.fetchone()
            
            if datos_log:
                dias_actuales= datos_log.get('dias',0) or 0
                ultimafecha= datos_log.get('ultimoingreso')
                
                fechaactual= datetime.now().date()
                
                if ultimafecha and isinstance(ultimafecha, datetime):
                    ultimafecha = ultimafecha.date()
                    
                if ultimafecha != fechaactual:
                    nuevosdias = dias_actuales +1
                    sql_update="UPDATE usuarios SET dias=%s, ultimoingreso= %s WHERE id_usuario =%s"
                    cursor.execute(sql_update,(nuevosdias, fechaactual, usuario_id))
                    conexion.commit()
                    dia=nuevosdias
                else:
                    dia=dias_actuales
        ...
        conexion.close()
    return render_template("perfil/perfilUsuario.html", nombre_usuario=session['nombre'], email=email, progreso_unidad1=progreso_porcentaje,diascurso=dia)
...
def render_module_page(user_id, col_name, template_name):
    ...
    try:
        with conexion.cursor() as cursor:
            sql = f"SELECT {col_name} FROM progreso_unidad1 WHERE id_usuario = %s"
    ...
    if unidad_completa == "true" and unit_id in ['1', '2', '3']:
        ...
        try:
            with conexion.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT id_progreso, unidad_1_completa, unidad_2_completa, unidad_3_completa FROM progreso_unidad1 WHERE id_usuario = %s", (usuario_id,))
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    sql_insert = """
                        INSERT INTO progreso_unidad1 (id_usuario, unidad_1_completa, unidad_2_completa, unidad_3_completa, progreso_total)
                        VALUES (%s, %s, %s, %s, 33)
                    """
                    cursor.execute(
...
def generate_pdf():
    ...
    curso = "Curso de Algebra"
    meses = ["", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
             "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"]
    mes_nombre = meses[datetime.now().month]
    fecha = f"{datetime.now().day} de {mes_nombre} del {datetime.now().year}"
    # Renderizar HTML con Jinja2
    rendered = render_template(
        'diploma/diploma.html', nombre=nombre, curso=curso, fecha=fecha)
